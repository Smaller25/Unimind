...
def fmri_transform(x, sparse_rate=0.0):
    # x: 1, num_voxels
...
def main(config):
    ...
    # create model
    # num_voxels = (sd['model']['pos_embed'].shape[1] - 1)* config_pretrain.patch_size                  # pos_embed이 없음
    ...
    model = MAEforFMRI(num_voxels=config.num_voxels, patch_size=config.patch_size, kernel_size = config.kernel_size, embed_dim=config.embed_dim,
    ...
    encoder_state_dict = sd['model']
    model_state_dict = torch.load(config.pretrain_model_path , map_location='cpu')['model']
    for key in encoder_state_dict.keys():
        model_state_dict[key] = encoder_state_dict[key]
    ...
    sampler = torch.utils.data.DistributedSampler(test_set, shuffle = True) if torch.cuda.device_count() > 1 else torch.utils.data.RandomSampler(test_set) 
