...
def pool_voxels(voxels, length):
    voxels = F.interpolate(voxels, size=length, mode='linear', align_corners=False)
    return voxels

class NSDDataset(Dataset):
    def __init__(self, root_dir, folder_list=None):
        self.root_dir = root_dir
        self.folder_list = folder_list if folder_list else []
        self.subj = self.subj_mapping()
        self.samples = self._load_samples()
        
    def _load_samples(self):
        if os.path.isfile(f'{self.root_dir}/classifier_NSDDatasetImage.npy'):
            samples = np.load(f'{self.root_dir}/classifier_NSDDatasetImage.npy', allow_pickle=True)
            ...
            for folder in self.folder_list:
                for file in os.listdir(os.path.join(self.root_dir, folder)):
                    if "nsdgeneral.npy" in file:
                        samples.append({'subj' : self.subj[folder], 'voxels' : self._load_npy(os.path.join(self.root_dir, folder, file))})
                    else:
                        pass
                print(f'folder_{folder} is done!!')
            np.save(f'{self.root_dir}/classifier_NSDDatasetImage.npy', samples)    
...
class NSDDataset2(Dataset):
    def __init__(self, root_dir, pool_num=8192, pool_type="avg", folder_list=None):
    ...
    def _load_samples(self):
        ...
        for folder in self.folder_list:
            for file in os.listdir(os.path.join(self.root_dir, folder)):
                if "nsdgeneral.npy" in file:
                    samples.append({'subj' : self.subj[folder], 'voxels' : self.vox_process(self._load_npy(os.path.join(self.root_dir, folder, file)))})
                else:
                    pass
        return samples
...
class NSDDatasetAE(Dataset):
    ...
    def _load_samples(self):
        ...
        for folder in self.folder_list:
            for file in os.listdir(os.path.join(self.root_dir, folder)):
                if "nsdgeneral.npy" in file:
                    samples.append(self._fmri_transform(self._load_npy(os.path.join(self.root_dir, folder, file)), 15724))
                else:
                    pass
        return samples
    ...
    def _fmri_transform(self, voxels, fmri_max_len):
        if len(voxels) != fmri_max_len:
            if isinstance(voxels, np.ndarray):
                fmris_tensor = torch.from_numpy(voxels)
            else:
                fmris_tensor = voxels

            # 텐서로 변환된 fmris_tensor에 대해 interpolate 적용
            voxels = F.interpolate(fmris_tensor.unsqueeze(0), size=fmri_max_len, mode='linear', align_corners=False)
        return voxels.squeeze()
    
    
    
class NSDDatasetImage(Dataset):
    def __init__(self, root_dir, folder_list=None, transform = lambda x:x):
        self.root_dir = root_dir
    ...
    def _load_samples(self):
        # 저장된 파일이 있으면 여기에서 가져오자
        if os.path.isfile(f'{self.root_dir}/diffusion_NSDDatasetImage.npy'):
            samples = np.load(f'{self.root_dir}/diffusion_NSDDatasetImage.npy', allow_pickle=True)
            print("dataset has already existed!!!!!!!!!!!!!")

        else:
            samples = []
            self.fmri_max_len = 15724

            for folder in self.folder_list:
                for file in os.listdir(os.path.join(self.root_dir, folder)):
                    if "nsdgeneral.npy" in file:
                        img_path = os.path.join(self.root_dir, folder, file[:-15] + '.jpg')
                        self.image = self.transform(self._load_image(img_path))
                        self.fmri = self._fmri_transform(np.load(os.path.join(self.root_dir, folder, file), allow_pickle=True), self.fmri_max_len)

                        samples.append({'fmri':self.fmri, 'image': self.image})

                        if self.fmri_max_len < self.fmri.shape[-1]:
                            self.fmri_max_len = self.fmri.shape[-1]
                print(self.fmri.shape)
            
            np.save(f'{self.root_dir}/diffusion_NSDDatasetImage.npy', samples)
        return samples

            
            # idx_list = list(range(len(fmris)))
            # if len(fmris) % 2 == 1:
            #     print('total dataset number is odd, so add the last sample once more')
            #     idx_list.append(idx_list[-1])
            #     print(idx_list)
            # idx_list = np.random.choice(idx_list, len(idx_list), replace=False)

            # for i in range(len(idx_list)//2):
            #     self.fmri1 = self._fmri_transform(fmris[idx_list[2*i]], self.fmri_max_len)
            #     self.fmri2 = self._fmri_transform(fmris[idx_list[2*i+1]], self.fmri_max_len)
            #     self.img1 = self.image_transform(images[idx_list[2*i]])
            #     self.img2 = self.image_transform(images[idx_list[2*i+1]])

            #     self.total_data.append({'fmri1': self.fmri1, 'image1': self.img1, 'fmri2': self.fmri2, 'image2': self.img2})

            # np.save(f'{self.root_dir}/diffusion_NSDDatasetImage.npy', self.total_data)    
...
class NSDDatasetSiam(Dataset):
    def __init__(self, root_dir, folder_list=None, image_transform = lambda x:x):
        self.root_dir = root_dir
    ...
    def _load_samples(self):
        # 저장된 파일이 있으면 여기에서 가져오자
        if os.path.isfile(f'{self.root_dir}/processed_NSDDatasetImage.npy'):
            self.total_data = np.load(f'{self.root_dir}/processed_NSDDatasetImage.npy', allow_pickle=True)
            print("dataset has already existed!!!!!!!!!!!!!")
            ...
            for folder in self.folder_list:
                for file in os.listdir(os.path.join(self.root_dir, folder)):
                    if "nsdgeneral.npy" in file:
                        ...
                        self.fmri = np.load(os.path.join(self.root_dir, folder, file), allow_pickle=True)
                        fmris.append(self.fmri)
                        if self.fmri_max_len < self.fmri.shape[-1]:
                            self.fmri_max_len = self.fmri.shape[-1]
                print(self.fmri.shape)
                print("Im doing very well")

            if len(fmris) != len(images):
                raise ValueError('number of fmri and image is different!!!')
            ...
            np.save(f'{self.root_dir}/processed_NSDDatasetImage.npy', self.total_data)    
            return self.total_data
...
class hcp_dataset(Dataset):
    def __init__(self, path='../data/HCP/npz', roi='VC', patch_size=16, transform=identity, aug_times=2, 
        ...
        if include_hcp:
            for c, (root, dir, files) in enumerate(os.walk(path)):
                for dirs in dir:
                    for file in os.listdir(os.path.join(path, dirs)):
                        ...
                        if file.endswith('.npz'):
                            npz = dict(np.load(os.path.join(root, dirs, file), allow_pickle=True))
...
# Siamese Dataset with GOD dataset
def create_Siamese_dataset(path='../data/webdataset_avg_split/',  roi='VC', patch_size=16, fmri_transform=identity,
            image_transform=identity, subjects = ['sbj_1', 'sbj_2', 'sbj_3', 'sbj_4', 'sbj_5'], 
            test_category=None, include_nonavg_test=False):
    img_npz = dict(np.load(os.path.join(path, 'images_256.npz')))
    with open(os.path.join(path, 'imagenet_class_index.json'), 'r') as f:
    ...
    if isinstance(image_transform, list):
        return (Siamese_dataset(train_fmri, train_img, train_img_label_all, fmri_transform, image_transform[0], num_voxels, len(npz['arr_0'])), 
                Siamese_dataset(test_fmri, test_img, test_img_label_all, torch.FloatTensor, image_transform[1], num_voxels, len(npz['arr_2'])))
    else:
        return (Siamese_dataset(train_fmri, train_img, train_img_label_all, fmri_transform, image_transform, num_voxels, len(npz['arr_0'])), 
                Siamese_dataset(test_fmri, test_img, test_img_label_all, torch.FloatTensor, image_transform, num_voxels, len(npz['arr_2'])))
...
class Siamese_dataset(Dataset):
    ...
    def __getitem__(self, index):
        index1 = index
        index2 = int(np.random.randint(len(self.fmri)))
        fmri1 = self.fmri[index1]
        fmri2 = self.fmri[index2]
        if index1 >= len(self.image):
            img1 = np.zeros_like(self.image[0])
        else:
            img1 = self.image[index1]
            
        if index2 >= len(self.image):
            img2 = np.zeros_like(self.image[0])
        else:
            img2 = self.image[index2]

        fmri1 = np.expand_dims(fmri1, axis=0) # (1, num_voxels)
        fmri2 = np.expand_dims(fmri2, axis=0) # (1, num_voxels)
        
        img1 = torchvision.transforms.ToTensor()(img1)
        img2 = torchvision.transforms.ToTensor()(img2)
        
        return {'fmri1': self.fmri_transform(fmri1), 'image1': self.image_transform(img1), 'fmri2': self.fmri_transform(fmri2), 'image2': self.image_transform(img2)}
...
class DummyDataset(Dataset):
    def __init__(self):
        self.samples = [{'image' : torch.zeros((3,256,256)), 'fmri' : torch.zeros((3, 8192))} for _ in range(1000)]
