...
from autoencoder.autoencoder import fmri_encoder, ConvMixer, ConvMixerAutoEncoder
from classifier.classifier import SubjectClassifier, ClassifierEncoder

def create_encoder_from_config(config):
    model = fmri_encoder(in_chans = 3, patch_size=config.patch_size, kernel_size = config.kernel_size, embed_dim=config.embed_dim, depth=config.depth) 
    return model

def create_classifier_from_config(config):
    model = ClassifierEncoder(in_chans = 3, dim = config.dim, depth = config.depth, kernel_size = config.kernel_size, patch_size = config.patch_size, n_classes = 4) 
    return model

class cond_stage_model(nn.Module):
    def __init__(self, metafile_encoder, metafile_classifier, cond_dim=1280):
        super().__init__()
        # prepare pretrained fmri mae 
        self.encoder = create_encoder_from_config(metafile_encoder['config'])
        self.classifierencoder = create_classifier_from_config(metafile_classifier['config'])
        
        self.encoder.load_checkpoint(metafile_encoder['model'])
        self.classifierencoder.load_state_dict(metafile_classifier['model'], strict = False)

        self.fmri_latent_dim = self.encoder.embed_dim
        self.channel_mapper = nn.Sequential(
            nn.Conv1d(self.fmri_latent_dim , self.fmri_latent_dim  // 2, 1, bias=True),
            nn.Conv1d(self.fmri_latent_dim // 2, self.fmri_latent_dim // 4, 1, bias=True),
            nn.Conv1d(self.fmri_latent_dim // 4, 1, 1, bias=True)
        )
        self.aggregation = nn.AdaptiveAvgPool1d(self.fmri_latent_dim*2*2)
        self.dim_mapper = nn.Linear(self.fmri_latent_dim*2*2, cond_dim, bias=True)
    def forward(self, x):
        latent = self.encoder(x)
        subject_token = self.classifierencoder(x)
        
        latent_crossattn = torch.cat([latent, subject_token], dim=-1)
        latent_crossattn = self.channel_mapper(latent_crossattn)
        
        latent_crossattn = self.aggregation(latent_crossattn)
...
class fLDM:
    ...
    def __init__(self, metafile_encoder, metafile_classifier, collate_fn, device=torch.device('cpu'),
        ...
        model.cond_stage_model = cond_stage_model(metafile_encoder, metafile_classifier, self.cond_dim)
        ...
        self.metafile = metafile_encoder
    ...
    def finetune(self, trainers, dataset, test_dataset, bs1, lr1,
        ...
        print('\n##### Stage One: only optimize conditional encoders #####')
        dataloader = DataLoader(dataset, batch_size=bs1, shuffle=True, collate_fn = self.collate_fn, pin_memory = True, num_workers = 16)
        # test_loader = DataLoader(test_dataset, batch_size=len(test_dataset), shuffle=False, collate_fn = self.collate_fn, pin_memory = True, num_workers = 16)
        test_loader = DataLoader(test_dataset, batch_size=bs1, shuffle=False, collate_fn = self.collate_fn, pin_memory = True, num_workers = 16)
