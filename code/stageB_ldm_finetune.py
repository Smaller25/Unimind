...
from dataset import NSDDatasetImage, DummyDataset
...
def my_collate_gen(batch):
    ...
    for sample in batch:
        ...
        if diff>0:
            ...
            collate_img.append(sample['image'])
    return {'image' : torch.stack(collate_img).float(), 'fmri' : torch.stack(collate_voxels).float()}
...
def main(config):
    ...
    # fmri_latents_dataset_train = NSDDatasetImage(root_dir = config.train_data_root, folder_list=['subj01', 'subj02', 'subj05'], transform = img_transform_train)
    # print(len(fmri_latents_dataset_train))
    fmri_latents_dataset_test = NSDDatasetImage(root_dir = config.test_data_root, folder_list=['subj01', 'subj02', 'subj05'], transform = img_transform_test)
    
    # fmri_latents_dataset_train = DummyDataset()
    # fmri_latents_dataset_test = DummyDataset()


    # prepare pretrained mbm 
    pretrain_encoder_metafile = torch.load(config.pretrain_encoder_path, map_location='cpu')
    pretrain_classifier_metafile = torch.load(config.pretrain_classifier_path, map_location='cpu')
    # create generateive model
    generative_model = fLDM(pretrain_encoder_metafile, pretrain_classifier_metafile, my_collate_gen,
...
def create_trainer(num_epoch, precision=32, accumulate_grad_batches=2,logger=None,check_val_every_n_epoch=0):
    ...
    return pl.Trainer(devices = 4, strategy="ddp", accelerator=acc, max_epochs=num_epoch, logger=logger, 
            precision=precision, accumulate_grad_batches=accumulate_grad_batches,
            enable_checkpointing=False, enable_model_summary=False, gradient_clip_val=0.5,
            check_val_every_n_epoch=check_val_every_n_epoch, profiler = SimpleProfiler(), sync_batchnorm = True)
...
if __name__ == '__main__':
    ...
    if config.checkpoint_path is not None:
        encoder_meta = torch.load(config.encoder_path, map_location='cpu')
        classifier_meta = torch.load(config.classifier_path, map_location ='cpu')
        ckp = config.checkpoint_path
        config = encoder_meta['config']
