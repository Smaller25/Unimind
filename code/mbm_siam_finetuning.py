...
from dataset import create_Kamitani_dataset, create_BOLD5000_dataset, create_Siamese_dataset
from sc_mbm.mae_for_fmri import MAEforFMRI,fmri_encoder
from sc_mbm.trainer import train_one_epoch_siamese
from sc_mbm.trainer import NativeScalerWithGradNormCount as NativeScaler
from sc_mbm.utils import save_model
from siamese_networks import SiameseNet
from sc_mbm.models import Clipper
...
def get_args_parser():
    parser = argparse.ArgumentParser('MAE finetuning with LLaVa', add_help=False)
...
def main(config):
    ...
    output_path = os.path.join(config.root_path, 'results', 'fmri_finetune_with_LLaVa',  '%s'%(datetime.datetime.now().strftime("%d-%m-%Y-%H-%M-%S")))
    ...
    if config.dataset == 'GOD':
        _, test_set = create_Siamese_dataset(path=config.kam_path, patch_size=config_pretrain.patch_size, 
                                subjects=config.kam_subs, include_nonavg_test=config.include_nonavg_test)
    ...
    config.num_voxels = test_set.num_voxels
    ...
    # create model
    model = fmri_encoder(num_voxels=num_voxels, patch_size=config.patch_size, kernel_size = config.kernel_size, embed_dim=config.embed_dim,
                depth=config.depth)
    ...
    if torch.cuda.device_count() > 1:
        ...
        model = DistributedDataParallel(model, device_ids=[config.local_rank], output_device=config.local_rank, find_unused_parameters=True)
    ...
    if hidden:
        ...
        out_dim = clip_size
    clip_extractor = clip_extractor.to(device)
    
    
    
    
    print('Finetuning MAE on test fMRI with LLaVA... ...')
    for ep in range(config.num_epoch):
        if torch.cuda.device_count() > 1: 
            sampler.set_epoch(ep) # to shuffle the data at every epoch
        loss = train_one_epoch_siamese(model, clip_extractor, dataloader_hcp, optimizer, device, ep, loss_scaler, logger, config, start_time, model_without_ddp)
